...
const country = ({ country }) => {
	return (
		<>
			<Meta title={country.map((i) => i.name.common)} />
			{country &&
				country.length > 0 &&
...
export async function getServerSideProps({ params }) {
	const req = await fetch(
		`https://restcountries.com/v3.1/name/${params.id}?fields=name,area,population,currencies,flags,translations,capital,languages,region,continents,maps,gini`
	)
